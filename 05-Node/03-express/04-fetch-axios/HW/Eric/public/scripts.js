const stud = document.querySelector(
  ".containerPage__containerForm1__form1--stud"
);
//btn
const btnGetDataQuery = document.querySelector(
  ".containerPage__containerForm2--submitQuerys"
);
const btnGetDataParams = document.querySelector(
  ".containerPage__containerForm2--submitParams"
);
//inputID
...
function loadStudents(ev) { //YS: Why do you have two functions that are exactly the same? 
  let html = "";
  ev.preventDefault();
  const list = document.getElementById("root");
  axios.get("/getAllStudents").then(({ data }) => {
...
function render(array) { //YS: Why do you have two functions that are exactly the same? 
  let html = "";
  const list = document.getElementById("root");
