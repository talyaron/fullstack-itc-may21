...
async function getStudentParam() {
  if (!searchId.value) {
    ...
    const studentInfoParam = await axios.get(
      `/showStudentParam/${searchId.value}`
    );
    studentInfoParam.data[0] === null
      ? alert("Please enter a valid ID")
      : render(studentInfoParam.data); //YS: Instead of having this, you should have a try catch. try { axios.get()....} catch {e}
  }
}
...
async function getStudentQuery() {
  if (!searchId.value) {
    ...
    const studentInfoQuery = await axios.get(
      `/showStudentQuery?id=${searchId.value}`
    ); //YS: Try / Catch with Async/Await
    studentInfoQuery.data[0] === null
      ? alert("Please enter a valid ID")
...
async function removeStudent(studentId, name) {
  try {
    ...
    if (option) {
      const DataWithOutDelete = await axios.delete(
...
async function sortTable(orderBy) {
  try {
    const getStudentsSorted = await getAllStudentsSorted(orderBy); //YS: You can just axios fetch here too instead of using a promise.
