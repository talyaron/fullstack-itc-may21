...
function userCookieWrite(req, res, next) {
  try {
    ...
    var _a = req.body,
        ...
        email = _a.email;
    if (!username || !email) throw new Error("User details processing issues"); //Here I set the cookie

    var cookieToWrite = JSON.stringify({
      username: username,
      email: email
    });
    ...
    req.cookieExists = true;
    req.username = username;
    req.email = email; //"Next" means that I will continue with the Route
