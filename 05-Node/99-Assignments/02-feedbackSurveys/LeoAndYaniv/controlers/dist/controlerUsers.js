...
function newUser(req, res) {
    try {
        ...
        var allUsers = new users_1.Users(); //YS: Would be better to create one global instance of the class so that everytime you use it it is the same instance. 
...
function login(req, res) {
    try {
        ...
        if (userExists) {
            ...
            //Set the cookie
            var cookieToWrite = JSON.stringify({ username: username, email: email }); //YS: No private info on cookies (you can use ID)
...
function uploadSurvey(req, res) {
    try {
        ...
        var newSurvey = new surveys_1.Survey(allSurveys.surveys[allSurveys.findSurveyIndex(uuid)]); //YS: Messy parameter - should make it into a variable
