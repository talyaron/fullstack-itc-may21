...
function render(array) {
  ...
  if (array && array.length > 0) {
    ...
    array.forEach((elem) => {
      const { name, email, address, phone, id } = elem;
    ...
    let html = ""; //YS: Why are you defining html again?
  }
  root.innerHTML = html;
}
...
function handleSubmit(event) {
  ...
  let { name, email, address, phone } = event.target.elements;
  name = name.value;
  email = email.value;
  ...
  if (name !== "" || email !== "" || address !== "" || phone !== "") {
    axios({
      method: "post",
      url: `/addEmployes`, //YS: Should pass the ID in params!
      data: {
        name,
        email,
        address,
...
async function deleteEmploye(id) {
  try {
    const option = confirm(`Are you sure do you want to delete this employe?`); //YS: NICE!!!!
    if (option) {
...
async function handleEdit(event) {
  event.preventDefault();
  try {
    let id = currentId; //YS: Why dont you just use the currentId?
    ...
    const employeData = await axios.put(`/updateEmployes`, updateEmploye); //YS: Id through params!
    event.target.reset();
