...
class Contact {
  ...
  time: Date;
  id: string;
  constructor(image: string, name: string, message: string, time: Date) {
    this.image = image;
    this.name = name;
...
class ContactList {
  ...
  render(): void {
    try {
      let rootDiv: HTMLDivElement = document.querySelector(
        ".saved-contacts-root"
      );
  ...
  remove(id: string): void {
    try {
      ...
      let contactDeserialized: string = JSON.parse(
        localStorage.getItem("whatsAppContacts")
      ); //YS: This is not being used.
    } catch (error) {
      console.error(error);
    }
  }
...
const handleSubmit = (event: any): void => {
  try {
    ...
    const newContact = new Contact(img, name, message, time);
...
const closeMenuModal = (event: MouseEvent): void => {
  //YS: You are not using the event, you dont need it
  try {
    const body: any = document.getElementsByName("body"); //YS: not being used
    const topModal: HTMLDivElement = document.querySelector(".top-modal");
