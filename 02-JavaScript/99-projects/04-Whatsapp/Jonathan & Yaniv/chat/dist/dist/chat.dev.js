...
var containerContactUser = document.querySelector('.container__header__left'); //modal

var btnModal = document.querySelector('.container__chat-footer--smile');
var bgModal = document.querySelector('.modal-bg');
var modalClose = document.querySelector('.modal-close'); //Ratio

var emojiList = document.querySelectorAll('.emoji'); //clicked
...
function () {
  ...
  Group.prototype.renderGroupChat = function () {
    try {
      ...
      var groupUser = this.groupUsers.filter(function (group) {
        return !group.match(contactUser);
      });
      ...
      if (groupUser.length === 2) {
        ...
        showGroupUser = this.groupUsers.filter(function (group) {
          return !group.match(contactUser);
        });
      }
...
function () {
  ...
  User.prototype.addMessages = function (newMess, groupId) {
    try {
      var groupIndex = this.userGroups.findIndex(function (group) {
        return group.groupId === groupId;
      }); //YS: You should use find instead of findIndex
  ...
  User.prototype.filterByMessage = function (inputMessageFilter, groupid) {
    try {
      var groupIndex = this.userGroupNew.findIndex(function (group) {
        return group.groupId === groupid;
      });
      var regrExp = "^" + inputMessageFilter;
      var searchTermReg_1 = new RegExp(regrExp, 'i');
      this.userGroups[groupIndex].groupMsgs = this.userGroupNew[groupIndex].groupMsgs.filter(function (elem) {
        return searchTermReg_1.test(elem.content);
      });
      this.renderMsgs(groupid);
    } catch (er) {
      console.error(er);
  ...
  User.prototype.editMessage = function (messageId, groupid) {
    try {
      ...
      var myMessageToEdit = this.userGroups[groupIndex].groupMsgs.filter(function (message) {
        return messageId === message.msgID;
      });
      myMessageToEdit[0].content = "you deleted this message";
      var myMessageToEditNew = this.userGroupNew[groupIndex].groupMsgs.filter(function (message) {
        return messageId === message.msgID;
      });
  ...
  User.prototype.handleDelete = function (messageId, groupid) {
    ...
    this.userGroups[groupIndex].groupMsgs = this.userGroups[groupIndex].groupMsgs.filter(function (message) {
      return message.content !== "you deleted this message";
    });
    this.userGroupNew[groupIndex].groupMsgs = this.userGroupNew[groupIndex].groupMsgs.filter(function (message) {
      return message.content !== "you deleted this message";
    });
...
function () {
  ...
  ContactList.prototype.findContactIndex = function (contactPhone) {
    try {
      var contactIndex = this.allContacts.findIndex(function (contactItem) {
        return contactItem.userPhone === contactPhone;
      });
      return contactIndex;
    } catch (er) {
      console.error(er);
