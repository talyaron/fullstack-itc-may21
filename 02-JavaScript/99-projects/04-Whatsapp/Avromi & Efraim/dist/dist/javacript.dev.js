...
function () {
  ...
  Contacts.prototype.renderContacts = function (domElement) {
    try {
      var html = this.contacts.map(function (contact) {
        return "<div class=\"holder__contact\" id=\"" + contact.contactId + "\" onclick=\"moveToPrivateChat('" + contact.contactId + "')\">" + ("<img class=\"holder__contact__image\" src=\"" + contact.imgUrl + "\">") + ("<div class=\"holder__contact__name\">" + contact.name + "</div>") + ("<div class=\"holder__contact__chat\">" + contact.chats[0].message + "</div>") + ("<div class=\"holder__contact__timestamp\">" + contact.chats[0].timeStamp + "</div>") + "<div class=\"holder__contact__unread\" id=\"unread\">6</div>" + ("<div class=\"holder__contact__unread\" id=\"delete\" onclick=\"deleteContact('" + contact.contactId + "')\">x</div>") + "</div>";
      }).join('');
  ...
  Contacts.prototype.findIndexes = function (contactID) {
    try {
      var index = this.contacts.findIndex(function (cnt) {
        return cnt.contactId === contactID;
      });
      return index;
    } catch (e) {
      console.error(e);
...
var deleteContact = function deleteContact(conatctID) {
  try {
...
var addToDomWithArray = function addToDomWithArray(searchResults) {
  try {
    ...
    searchResults.forEach(function (contact) {
      ...
      holder_1.innerHTML += "<div class=\"holder__contact\" id=\"" + contact.contactId + "\" onclick=\"moveToPrivateChat('" + contact.contactId + "')\">" + ("<img class=\"holder__contact__image\" src=\"" + contact.imgUrl + "\">") + ("<div class=\"holder__contact__name\">" + contact.name + "</a></div>") + ("<div class=\"holder__contact__chat\">" + contact.chats[index].message + "</div>") + ("<div class=\"holder__contact__timestamp\">" + contact.chats[index].timeStamp + "</div>") + "<div class=\"holder__contact__unread id=\"unread\">6</div>" + ("<div class=\"holder__contact__unread id=\"delete\" onclick=\"deleteContact('" + contact.contactId + "')\">x</div>") + "</div>";
    });
  } catch (e) {
    console.error(e);
  }
...
var findContactSearch = function findContactSearch(chatSearch, searchTerm) {
  try {
    var userRegEx_1 = new RegExp(searchTerm, 'gmi');
    var searchedUsers = chatSearch.filter(function (contactName) {
      return userRegEx_1.test(contactName.name);
    });
...
var findTextInMessages = function findTextInMessages(searchTerm) {
  try {
