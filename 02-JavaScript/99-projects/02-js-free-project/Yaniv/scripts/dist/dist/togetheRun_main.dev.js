...
/** @class */
function () {
  function Run(runDistance, runTime, runPace, runArea) {
    this.runMatch = false;
...
function () {
  ...
  RunsPool.prototype.addToPool = function (run) {
    ...
    this.allRuns = this.allRuns.sort(function (a, b) {
      return a.runRunnerId - b.runRunnerId;
    });
  ...
  RunsPool.prototype.findMatch = function (run) {
    var runMatches = this.allRuns.filter(function (runItem) {
      return runItem.runRunnerId !== run.runRunnerId && Math.abs((runItem.runTime.getTime() - run.runTime.getTime()) / (1000 * 60 * 60)) < 2 && runItem.runPace === run.runPace && runItem.runArea === run.runArea;
    });

    if (runMatches.length > 0) {
      run.runMatch = true;
      var currentRunIndex = this.allRuns.findIndex(function (runItem) {
        return runItem.runId === run.runId;
      });
  ...
  RunsPool.prototype.showMatchesOnDON = function (runMatches) {// in the future - the method will add a button to show matches to the run box, in case available.
...
function () {
  ...
  LoggedInRunner.prototype.personalDetailsToDOM = function () {
    ...
    runnerRunsCounter.innerHTML = "" + this.runnerRuns.length; // window.location.href = `togetheRun_main.html?${currentRunner.runnerId}`; // causes endless loop of loading the page...can be solved by localSession.setItem("isFirstLoad",false) during first loading of the page
  };

  LoggedInRunner.prototype.addRun = function (run) {
    this.runnerRuns.push(run);
    this.runnerRuns = this.runnerRuns.sort(function (a, b) {
      return a.runTime - b.runTime;
    });
...
var onlyFutureRuns = function onlyFutureRuns() {
  try {
...
var runSubmit = function runSubmit(ev) {
  try {
    ev.preventDefault();
    ...
    var run = new Run(runDistance, runTime, runPace, runArea);
